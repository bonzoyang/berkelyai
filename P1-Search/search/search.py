...
def depthFirstSearch_(problem):
    ...
    while not problem.isGoalState(state):
        # get all non-explored successor
        successor = [(s, a, c) for s, a, c in problem.getSuccessors(state) if s not in explored]
        explored.append(state)
        
        # if successor are all explored, backtrack
        if not successor:
            state = track.pop()
            action.pop()
            cost.pop()
            
        # if successor are not all explored, forwardtrack
        else:
            for s, a, c in successor:
                traverse.push((s, a, c)) 
            else:
                track.append(state)
                s, a, c = traverse.pop()
                ...
                cost.append(c)
                state = s
                print(f'from traverse, state={state}')
